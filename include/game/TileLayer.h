...
public:
    ...
    bool load(const std::string& tileset, const Tile* tiles, sf::Vector2u tileSize, size_t width, size_t height)
    {
        ...
        for (unsigned int i = 0; i < width; ++i)
            for (unsigned int j = 0; j < height; ++j)
            {
                ...
                int tileNumber = tiles[i + j * width].index;
    ...
    void updateTiles(const Tile* tiles, sf::Vector2u tileSize, size_t width, size_t height) {
        m_vertices.resize(width * height * 4);
    ...
    bool updateTexture(const std::string& tileset) {
